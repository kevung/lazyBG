<script>
    ...
    function startInserting() {
        ...
        hasAutoFocused = true;
        statusBarTextStore.set('INSERT MODE: Choose position and press Enter to insert empty decision, Esc=cancel');
    }
    ...
    async function validateInserting() {
        ...
        // Insert empty decision (will be flagged by validation)
        ...
        // Get the newly inserted move index
        const newMoveIndex = insertPosition === 'before' ? moveIndex : moveIndex + 1;

        // Invalidate cache and force recalculation
        await invalidatePositionsCacheFrom(gameIndex, newMoveIndex);
        
        // Select the newly inserted decision and activate edit mode
        selectedMoveStore.set({ gameIndex, moveIndex: newMoveIndex, player });
        
        const posText = insertPosition === 'before' ? 'before' : 'after';
        statusBarTextStore.set(`Empty decision inserted ${posText} at game ${gameIndex + 1}, move ${newMoveIndex}. Use Tab to edit.`);
